...
const testDir = join(homedir(), ".openclaw", "email-gmail", "contexts");
const testMsgIndexDir = join(homedir(), ".openclaw", "email-gmail", "msg-index");
...
describe("delivery-context", () => {
  ...
  it("writes and resolves message-ID index", async () => {
    const msgId = `<test-${Date.now()}@gmail.com>`;
